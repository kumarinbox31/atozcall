    ...
    $.magnificPopup.open({
        ...
        callbacks: {
            beforeClose: function() {
                $('body').find('.mfp-container').addClass('nasa-bot-to-top');
...
$('body').on('get_video_poster',function() {
    var _int_get_video_poster;
   


    _int_get_video_poster = setInterval(function() {
        var _video =  $('body').find('.item-wrap .nasa-item-main-video-wrap .wp-video-shortcode:not(.mejs-container)');
        var _video_thumb =  $('body').find('.nasa-wrap-item-thumb.ns-video-poster');
        
        if($(_video).length) {
            var video = $(_video)[0];

            var canvas = $('.ns-canvas')[0];
            var context = canvas.getContext('2d');

           

            video.onloadedmetadata = function() {
                // Set canvas size using video size
                canvas.width = video.videoWidth;
                canvas.height = video.videoHeight;

                // Starting from 0.1 second
            ...
            video.onseeked = function() {
                ...
                $(canvas).css({'display':'block','width':'100%'});
                $('.ns-video-size').remove();
                
                if($(_video_thumb).length) {
                    var canvas_thumb =  $('.ns-canvas-thumb')[0];
                    var context_thumb = canvas_thumb.getContext('2d');

                    canvas_thumb.width = video.videoWidth;
                    canvas_thumb.height = video.videoHeight;

                    context_thumb.drawImage(video, 0, 0, canvas.width, canvas.height);
                    $(canvas_thumb).css({'display':'block','width':'100%'});
                    $('.ns-video-load').remove();
                }
                ...
                clearInterval(_int_get_video_poster);
