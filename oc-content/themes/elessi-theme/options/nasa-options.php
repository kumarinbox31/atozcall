...
function elessi_enqueue_style() {
    ...
    $theme_version = isset($nasa_opt['css_theme_version']) && $nasa_opt['css_theme_version'] ? NASA_VERSION : null;
...
function elessi_enqueue_scripts() {
    ...
    $theme_version = isset($nasa_opt['js_theme_version']) && $nasa_opt['js_theme_version'] ? NASA_VERSION : null;
    ...
    if (NASA_WOO_ACTIVED) {
        ...
        if ($mini_cart_note || $mini_cart_shipping || $mini_cart_coupon) {
            ...
            wp_enqueue_script($prefix . '-ext-mini-cart', ELESSI_THEME_URI . '/assets/js/min/ext-mini-cart.min.js', array('jquery'), $theme_version, true);
