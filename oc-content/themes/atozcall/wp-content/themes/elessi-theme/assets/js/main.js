...
$('body').on('click', '.toggle-sidebar-shop', function() {
    $('.transparent-window').fadeIn(200).addClass('desk-window');
...
$('body').on('click', '.black-window, .white-window, .transparent-desktop, .transparent-mobile, .transparent-window, .nasa-close-mini-compare, .nasa-sidebar-close a, .nasa-sidebar-return-shop, .login-register-close, .nasa-close-menu-mobile', function() {
    ...
    if ($('.nasa-single-product-in-mobile #reviews #review_form_wrapper .ns-form-close').length) {
        $('.nasa-single-product-in-mobile #reviews #review_form_wrapper .ns-form-close').trigger('click');
    }
    ...
    if ($('body').hasClass('nasa-quickview-on')) {
        if (
            ...
            if (!$(_this).hasClass('btn-from-wishlist')) {
                ...
                if ($(_this).hasClass('nasa-before-click')) {
                ...
                else if ($(_this).hasClass('nasa-quick-add')) {
                    ...
                    if ($(_parent).find('.variations_form').length) {
                        $(_parent).addClass('ns-var-loaded');
                    } else {
        ...
        if ($(_this).hasClass('nasa-before-click')) {
            ...
            if ($(_this).hasClass('nasa-quick-add')) {
                if ($(_parent).hasClass('out-of-stock')) {
                    var _href = $(_this).attr('href');
                    if (_href) {
                        window.location.href = _href;
...
$('body').on('click','.ns-close-quickadd', function(e) {
    ...
    $(_pr_item).removeClass('ns-var-loaded');
...
$('body').on('click','.popup-cart-close, .popup-cart-bg-close', function(e) {
    ...
    $(_pr_item).addClass('ns-bottom-to-top');
    
    setTimeout(function() {
        $(_pr_item).removeClass('ns-bottom-to-top nasa-active').hide();
...
$('body').on('added_to_cart', function(ev, fragments, cart_hash, _button) {
     ...
     * Close quick-view
     */
    if ($('.ns-cart-popup-wrap').length <= 0) {
        $.magnificPopup.close();
    }
...
if ($('.header-type-builder').length && $('.nasa-nav-extra-warp').length <= 0) {
    $('body').append('<div class="nasa-nav-extra-warp nasa-show"><div class="desktop-menu-bar"><div class="mini-icon-mobile"><a href="javascript:void(0);" class="nasa-mobile-menu_toggle bar-mobile_toggle"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="26px" height="24px" fill="currentColor"><path d="M 4 7 L 4 9 L 28 9 L 28 7 Z M 4 15 L 4 17 L 28 17 L 28 15 Z M 4 23 L 4 25 L 28 25 L 28 23 Z"></path></svg></a></div></div></div>');
}
 ...
 */
$('body').on('wc_fragments_refreshed nasa_opened_cart_sidebar wc_fragments_loaded', function() {
    if ($('#cart-sidebar .cross-sells').length) {
        var _cross_sells= $('#cart-sidebar .cross-sells').find('.nasa-slick-slider');
        $(_cross_sells).attr('data-columns', '3');
        $(_cross_sells).attr('data-columns-tablet', '3');
        $(_cross_sells).attr('data-columns-small', '3');

        if (!$(_cross_sells).hasClass('nasa-nav-top')) {
            $(_cross_sells).addClass('nasa-nav-top');
        }
        
        if ($('#cart-sidebar .cross-sells .product-item').length) {
            $('#cart-sidebar .cross-sells .product-item').each(function() {
                var _this = $(this);
        ...
        if (!$(_cross_sells).hasClass('nasa-ver-buttons')) {
            $(_cross_sells).addClass('nasa-ver-buttons');
        }
...
$('body').on('touchstart', '#cart-sidebar, #nasa-wishlist-sidebar, #nasa-quickview-sidebar, #nasa-menu-sidebar-content, .nasa-top-cat-filter-wrap-mobile, .nasa-top-sidebar, .account-nav-wrap.vertical-tabs', function(e) {
    if (!$(e.target).hasClass('slick-active')) {
        touchstart = e.touches[0].clientX;
    }
});

$('body').on('touchend', '#cart-sidebar, #nasa-wishlist-sidebar, #nasa-quickview-sidebar', function(e) {
