...
jQuery(document).ready(function($) {
    ...
    $('body').on('ns_add_read_more', function() {
        if ($('.woocommerce-tabs .ns-tab-item').length && $('#tmp-read-more-tab').length) {
            $('.woocommerce-tabs .ns-tab-item').each(function() {
                ...
                if ($(_this).find('.nasa-content-accessories_content, .nasa-content-additional_information').length) {
    ...
    $('body').on('touchstart', '.variations_form, #nasa-content-size-guide, #nasa-content-delivery-return, #nasa-content-ask-a-quetion, #review_form_wrapper', function(e) {
        if ($(e.target).attr('id') != 'comment') {
            touchstart = e.touches[0].clientY;
        }
    });
    
    $('body').on('touchend', '.variations_form, #nasa-content-size-guide, #nasa-content-delivery-return, #nasa-content-ask-a-quetion, #review_form_wrapper', function(e) {
