...
jQuery(document).ready(function($) {
    ...
    $('body').on('click', '.quick-view', function(e) {
        ...
        if($('.ns-cart-popup-wrap').length && $('.ns-cart-popup-wrap').hasClass('nasa-active')) {
            $('.ns-cart-popup-wrap').addClass('ns-bottom-to-top');
            setTimeout(function() {
                $('.ns-cart-popup-wrap').removeClass('ns-bottom-to-top nasa-active').hide();
            },200);
        }
                        ...
                        success: function(response) {
                            ...
                            // Popup classical
                                ...
                                if ($('#nasa-quickview-popup').length < 1) {
                                    $('body').append('<div id="nasa-quickview-popup" class="product-lightbox nasa-bot-to-top"></div>');
                                }
        ...
        if (_countSelect && _selected === _countSelect) {
            ...
            if ($('.product-lightbox .woosb-product').length) {
                ...
                if($(_form).parents('.product-item').length) {
                    $(_form).parents('.product-item').find('.main-img img:first-child').attr('src', _src_large);
                }
            }
        }
